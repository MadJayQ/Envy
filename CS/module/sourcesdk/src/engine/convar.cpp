...
#define stackalloc( _size )		_alloca( ALIGN_VALUE( _size, 16 ) )
		...
		public:
			virtual bool RegisterConCommandBase(ConCommandBase *pVar)
			{
				...
				(*Interfaces::Instance()->GetInterface<ICvar>())->RegisterConCommand(pVar);
				return true;
			}
		};
		...
		{
			if (!(Interfaces::Instance()->GetInterface<ICvar>()) || s_bRegistered)
				return;

			assert(s_nDLLIdentifier < 0);
			s_bRegistered = true;
			s_nCVarFlag = nCVarFlag;
			s_nDLLIdentifier = (*Interfaces::Instance()->GetInterface<ICvar>())->AllocateDLLIdentifier();
		...
		void ConVar_Unregister()
		{
			...
			assert(s_nDLLIdentifier >= 0);
			(*Interfaces::Instance()->GetInterface<ICvar>())->UnregisterConCommands(s_nDLLIdentifier);
			s_nDLLIdentifier = -1;
			s_bRegistered = false;
		}
		...
		{
			if ((Interfaces::Instance()->GetInterface<ICvar>())) {
				(*Interfaces::Instance()->GetInterface<ICvar>())->UnregisterConCommand(this);
			}
		}
		...
		void ConVar::ChangeStringValue(const char *tempVal, float flOldValue)
		{
			...
			if ((Interfaces::Instance()->GetInterface<ICvar>()))
				(*Interfaces::Instance()->GetInterface<ICvar>())->CallGlobalChangeCallbacks(this, pszOldValue, flOldValue);
		}
