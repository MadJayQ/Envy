...
namespace Envy
{
	namespace SourceEngine
	{
		...
		public:
			...
			inline bool Entrust()
			{
				while (viewangles.yaw <= -180) viewangles.yaw += 360;
				while (viewangles.yaw > 180) viewangles.yaw -= 360;
				while (viewangles.pitch <= -180) viewangles.pitch += 360;
				while (viewangles.pitch > 180) viewangles.pitch -= 360;

				if (viewangles.pitch > 89.f)
					viewangles.pitch = 89.f;
				if (viewangles.pitch < -89.f)
					viewangles.pitch = -89.f;
				if (viewangles.yaw > 180.f)
					viewangles.yaw = 180.f;
				if (viewangles.yaw < -180.f)
					viewangles.yaw = -180.f;

				viewangles.roll = 0.f;

				if (isnan(viewangles.pitch) || isinf(viewangles.pitch) ||
					isnan(viewangles.yaw) || isinf(viewangles.yaw) ||
					isnan(viewangles.roll) || isinf(viewangles.roll)) 
				{
					viewangles = QAngle(0.f, 0.f, 0.f);
					return false;
				}

				if (forwardmove < -450) forwardmove = -450;
				if (sidemove < -450) sidemove = -450;
				if (forwardmove > 450) forwardmove = 450;
				if (sidemove > 450) sidemove = 450;
