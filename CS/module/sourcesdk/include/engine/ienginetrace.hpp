...
namespace Envy
{
	namespace SourceEngine
	{
		...
		public:
			...
			{
				return BaseShouldHitEntity((IHandleEntity*)pSkip, pEntity, m_CollisionGroup, contentsMask);
			}
		...
		public:
			...
			virtual bool ShouldHitEntity(IHandleEntity *pEntityHandle, int contentsMask)
			{
				if (!PassServerEntityFilter(pEntityHandle, m_pPassEnt2))
					return false;
				return BaseShouldHitEntity(pEntityHandle, pSkip, m_CollisionGroup, contentsMask);
			}
	...
	class IEngineTrace : public SourceInterface<SourceEngine::_IEngineTrace_>
	{
	public:
		IEngineTrace() {}
		...
		virtual Interface* Init() override
		{
			m_data = reinterpret_cast<Interface*>(
				Interfaces::Instance()->GetInterfaceAddr("EngineTraceClient")
			);

			SourceInterface<SourceEngine::_IEngineTrace_>::Init();
