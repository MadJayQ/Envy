...
#define SIGNONSTATE_CHANGELEVEL	7	// server is changing level, please wait
	...
	class IVEngineClient : 
		public SourceInterface<SourceEngine::_IVEngineClient_>
	{
	public:
		IVEngineClient() {}
		...
		virtual Interface* Init() override
		{
			m_data = reinterpret_cast<Interface*>(
				Interfaces::Instance()->GetInterfaceAddr("VEngineClient")
			);
			SourceInterface<SourceEngine::_IVEngineClient_>::Init();

			m_ClSendMoveAddr = (uintptr_t)Peb::Instance()->GetModule("engine.dll").FindPattern("55 8B EC A1 ? ? ? ? 81 EC ? ? ? ? B9 ? ? ? ? 53 8B 98");
			return m_data;
		}

		void CL_SendMove()
		{
			Function<void> sendmove(m_ClSendMoveAddr);
			sendmove();
		}
	private:
		uintptr_t m_ClSendMoveAddr;
	};
}