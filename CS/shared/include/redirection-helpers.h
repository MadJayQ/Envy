...
public:
	RedirectionHelper(RedirectionHelper const&) = delete;
	RedirectionHelper& operator=(RedirectionHelper&) = delete;

	static RedirectionHelper* Instance()
	{
		static std::shared_ptr<RedirectionHelper> s_pInstance{ new RedirectionHelper };
		return s_pInstance.get();
	}

	NTSTATUS Resolvepath(std::string& path, const std::string& name, const std::string& search_directory, Process& proc, HANDLE actx);
	uintptr_t ResolvePath(std::wstring& path, ResolveFlags flags, Process& proc, const std::wstring& baseName = L"");

	uintptr_t ProbeSxSRedirect(std::wstring& path);


	std::string ConvertFromWide(const std::wstring& str) { return converter.to_bytes(str); }
	std::wstring ConvertFromMBS(const std::string& mbs) { return converter.from_bytes(mbs); }
...
private:
	...
	std::unordered_map<std::string, std::vector<std::string>> api_schema;
